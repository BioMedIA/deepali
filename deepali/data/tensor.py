r"""Subclass of tensor-like tensor wrappers with additional meta-data."""
...
class TensorDecorator(object):
    r"""Tensor decorator base class."""

    __slots__ = ("_tensor",)

    def __init__(self: T, data: Tensor) -> None:
        r"""Initialize wrapped tensor."""
        if not isinstance(data, Tensor):
            raise TypeError(f"{type(self).__name__}() 'data' must be a torch.Tensor")
        self._tensor = data

    @overload
    def tensor(self: T) -> Tensor:
        r"""Get wrapped tensor."""
        ...

    @overload
    def tensor(self: T, data: Tensor, **kwargs) -> T:
        r"""Get shallow copy which wraps given tensor."""
        ...
    ...
    def tensor(
        ...
        **kwargs,
        ...
        r"""Get wrapped tensor or shallow copy which wraps given tensor."""
        if data is None:
            return self._tensor
        return shallow_copy(self).tensor_(data, **kwargs)

    def tensor_(self: T, data: Tensor) -> T:
        r"""Replace wrapped tensor."""
    ...
    def dtype(self: T) -> torch.dtype:
        r"""Type of image data."""
        return self._tensor.dtype

    @property
    def device(self: T) -> torch.device:
        r"""Device on which image data is stored."""
        return self._tensor.device

    @property
    def shape(self: T) -> torch.Size:
        r"""Shape of image data tensor."""
        return self._tensor.shape

    @property
    def data(self: T) -> Tensor:
        r"""Detached image data tensor, i.e., ``self.tensor().data``."""
        return self._tensor.data

    def detach(self: T) -> T:
        r"""Detach image data from computation graph.
    ...
    def to(self: T, *args, **kwargs) -> T:
        r"""Change dtype and/or device of data tensor.
    ...
    def pin_memory(self) -> T:
        r"""Put wrapped tensor data in pinned memory."""
        self._tensor = self._tensor.pin_memory()
        return self

    def is_pinned(self) -> bool:
        r"""Whether wrapped tensor is in pinned memory."""
        return self._tensor.is_pinned()

    def permute(self: T, *names: str) -> Tensor:
        r"""Image data tensor with permuted named dimensions."""
    ...
    def clamp(self: T, min: Optional[Scalar] = None, max: Optional[Scalar] = None) -> T:
        r"""Element-wise clamp values to specified min/max values."""
    ...
    def add(self: T, other: Union[Scalar, Array, TensorDecorator]) -> T:
        r"""Element-wise add values of this and other tensor or constant."""
    ...
    def add_(self: T, other: Union[Scalar, Array, TensorDecorator]) -> T:
        r"""Element-wise add values of other tensor or constant to this tensor."""
        if isinstance(other, TensorDecorator):
            other = other._tensor
        other = as_tensor(other, dtype=self.dtype, device=self.device)
        self._tensor += other
    ...
    def sub(self: T, other: Union[Scalar, Array, TensorDecorator]) -> T:
        r"""Element-wise subtract values of this and other tensor or constant."""
        if isinstance(other, TensorDecorator):
            other = other._tensor
        other = as_tensor(other, dtype=self.dtype, device=self.device)
        result = self._tensor - other
    ...
    def sub_(self: T, other: Union[Scalar, Array, TensorDecorator]) -> T:
        r"""Element-wise subtract values of other tensor or constant to this tensor."""
        if isinstance(other, TensorDecorator):
            other = other._tensor
        other = as_tensor(other, dtype=self.dtype, device=self.device)
        self._tensor -= other
    ...
    def mul(self: T, other: Union[Scalar, Array, TensorDecorator]) -> T:
        r"""Element-wise multiply values of this and other tensor or constant."""
        if isinstance(other, TensorDecorator):
            other = other._tensor
        other = as_tensor(other, dtype=self.dtype, device=self.device)
        result = self._tensor * other
    ...
    def mul_(self: T, other: Union[Scalar, Array, TensorDecorator]) -> T:
        r"""Element-wise multiply values of this tensor by values of other tensor or constant."""
        if isinstance(other, TensorDecorator):
            other = other._tensor
        other = as_tensor(other, dtype=self.dtype, device=self.device)
        self._tensor *= other
    ...
    def div(self: T, other: Union[Scalar, Array, TensorDecorator]) -> T:
        r"""Element-wise divide values of this by values of other tensor or constant."""
        if isinstance(other, TensorDecorator):
            other = other._tensor
        other = as_tensor(other, dtype=self.dtype, device=self.device)
        result = self._tensor / other
    ...
    def div_(self: T, other: Union[Scalar, Array, TensorDecorator]) -> T:
        r"""Element-wise divide values of this tensor by values of other tensor or constant."""
        if isinstance(other, TensorDecorator):
            other = other._tensor
        other = as_tensor(other, dtype=self.dtype, device=self.device)
        self._tensor /= other
    ...
    def __torch_function__(self: T, func, types, args=(), kwargs=None) -> T:
